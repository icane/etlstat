...
def parse_df_columns(data):
    """Parses columns in a dataframe: converts capital letters into lowercase
       letters, white spaces into low bar and removes accent marks.
       Args:
...
def replace_urls_in_xml(tree, urls_df, output_file):
    """Replaces URLs in Pentaho Data Integration (Kettlt) xml files. First
       thought for PC-Axis URLs, but it can be used with any generic URL.
...
def bulk_replace_url_in_xml(input_data_path, output_data_path=None):
    ...
    for job_dir in job_dirs:
        ...
        LOGGER.info("Using mapping file: %s", str(next(iter(mappings))))
